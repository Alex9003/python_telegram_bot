...
def all_notes(message):
    with get_bd_cursor() as cur:
        ...
        if row:
            ...
            if notes:
                bot.send_message(message.chat.id, 'Список нотатків\n' + notes)
            else:
...
def notification_note(call, note_id):
    ...
    m = """Введіть новий час в такому форматі:
...
def save_notification_note(message, note_id):
    ...
    try:
        ...
        with get_bd_cursor() as cur:
            ...
            if cur.rowcount > 0:
                bot.send_message(message.chat.id, 'Час оновлення!')
            else:
                bot.send_message(message.chat.id, 'Помилка оновлення :(')
    except Exception:
        bot.send_message(message.chat.id, 'Дивися що вводиш!')
